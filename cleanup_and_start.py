# -*- coding: utf-8 -*-
"""
Cleanup database and start monitoring for 03759.HK.
"""
...
def cleanup_database():
    """Cleanup all monitor data from database."""
    ...
    with db.get_session() as session:
        # Delete all alerts
        result = session.execute(delete(StockMonitorAlert))
        logger.info(f"Deleted {result.rowcount} alert(s)")

        # Delete all states
        result = session.execute(delete(StockMonitorState))
        logger.info(f"Deleted {result.rowcount} state(s)")

        # Delete all configs
        result = session.execute(delete(StockMonitorConfig))
        logger.info(f"Deleted {result.rowcount} config(s)")

        session.commit()


def start_monitoring():
    """Cleanup and start monitoring for 03759.HK."""
    ...
    logger.info("=" * 60)
    logger.info(f"Starting monitoring for {stock_code} (康龙化成)")
    logger.info("=" * 60)

    # Check Feishu configuration and set up client
    logger.info("\n--- Setting up Feishu client ---")
